...
test("Impostazioni > Privacy mostra dati reali dell'account e non richiede la master key", async ({
  ...
  await expect(page.getByText(/^0 promemoria\/scadenze /)).toBeVisible();
  await expect(page.getByText("0 fascicoli (0 aperti, 0 chiusi)")).toBeVisible();
  await expect(
    page.getByText("Per ogni contenuto in archivio: a quale categoria, bene e fascicolo"),
  ).toBeVisible();
