...
**Cosa fa:** in "Importa più file insieme" (`/archive/import`) compare, oltre a "Scegli i file da importare", un secondo bottone: **Importa da Google Drive**. Apre un file browser con la grafica di Hinthial (non il selettore di Google: la prima versione lo usava, ma la sua grafica non è personalizzabile --- v. "Note tecniche"), dove puoi navigare tra le cartelle del tuo Drive e scegliere singoli file o cartelle intere. Da quel momento i file scelti seguono esattamente lo stesso percorso di quelli scelti dal disco: stessa lettura del contenuto, stesso riconoscimento per emittente, stessa proposta di fascicolo, stessa cifratura all'importazione finale. In più: se importi una cartella intera, il suo nome viene usato come suggerimento di categoria per i file al suo interno (solo se il contenuto del file non ne ha già suggerita una migliore) --- un aiuto in più, non un nuovo modo di organizzare l'archivio: le cartelle di Google restano un dettaglio della provenienza, non diventano un costrutto permanente di Hinthial (i Fascicoli già coprono, meglio, quel bisogno --- v. discussione con l'utente).

**Note tecniche:** prima versione basata sul Picker di Google (scope `drive.file`), sostituita in giornata dopo aver visto che la sua grafica --- un iframe controllato da Google per motivi di sicurezza --- non è personalizzabile. La versione attuale (`domain/google-drive/client.ts`, `GoogleDriveBrowser.tsx`) interroga direttamente la Drive API v3 per disegnare la navigazione a cartelle, il che richiede lo scope più ampio `drive.readonly` --- dichiarato apertamente nella bozza di informativa (`PRIVACY_POLICY_DRAFT.md`) insieme alla verifica standard che Google richiede per questo scope prima che chiunque fuori dagli utenti di test possa usarlo (non l'audit di sicurezza annuale a pagamento riservato a scope "restricted" come Gmail, FASE 26). Tutto resta lato client: token OAuth (Google Identity Services, cache in memoria per un solo consenso a sessione) e ogni chiamata alla Drive API restano nel browser, il nostro server non vede né l'uno né le altre. Una cartella scelta per intero viene espansa a tutti i suoi file, a qualunque profondità nelle sottocartelle, solo al momento dell'importazione --- non durante la navigazione, per non moltiplicare le richieste. Non serve più la chiave API del Picker (rimossa dal codice, può restare inutilizzata su Google Cloud). `BulkImportForm.tsx` (FASE 21) resta refattorizzato per separare "come arrivano i file" da "cosa se ne fa" (`processFiles`, invariato).

**Limiti noti, dichiarati apertamente**: nessun rilevamento di duplicati "vero" (lo stesso file arrivato da più strade) --- resta un lavoro a parte. Nessun conteggio "N elementi" su una cartella non ancora aperta, né indicatore "parziale" sulle cartelle nella lista --- richiederebbe una richiesta per riga, costosa su un Drive con molte cartelle. Il flusso OAuth reale non è testabile in automatico (richiederebbe un account Google vero dentro un test e2e): verificato manualmente, non con un test dedicato.

Verificato: typecheck, lint, unit test rieseguiti senza modifiche, `bulk-import.spec.ts` passato senza modifiche dopo il refactoring.
...
**Cosa fa:** ripensato da zero il modello di Amici, discusso a fondo con l'utente prima di scrivere codice. Ogni contatto nasce **PERSONA** --- privato, un solo verso, esattamente come prima: nessuna notifica a nessuno, può comunque essere destinatario di capsule. Se quell'indirizzo corrisponde a un account Hinthial già esistente, compare il tasto **"Richiedi amicizia"**: manda un'email e un popup in Dashboard al destinatario, che può accettare o rifiutare. Se rifiuta, il mittente resta una PERSONA come prima. Se accetta, **entrambi** compaiono come **AMICO** ("🤝 Amico") nella rispettiva lista --- non solo nella propria. Solo un AMICO può ricevere una richiesta di **diventare guardiano** ("Chiedi di diventare guardiano"): anche questa richiede accettazione esplicita, non è più un interruttore che il proprietario azionava da solo su un amico ignaro. Chi accetta compare come guardiano ("🛡️ Guardiano") nella lista di chi l'ha chiesto, e vede quella persona come "protetto" nella nuova pagina **Protetti** (link "Vedi chi proteggi" in cima ad Amici) --- da lì può anche dimettersi in qualunque momento, senza bisogno del consenso del proprietario. Il proprietario può invece rimuovere un guardiano direttamente, senza chiedere permesso: serve consenso solo per assumere il ruolo, non per lasciarlo o toglierlo.

**Conseguenza per chi ha già usato la funzione:** ogni amico esistente (anche chi era già collegato o segnato guardiano nel vecchio modello, senza mai averlo saputo) diventa una PERSONA. Chi aveva già configurato Eredità digitale con dei guardiani si ritrova con zero guardiani confermati finché ciascuno non riceve e accetta la nuova richiesta --- una scelta esplicita dell'utente, sapendo che comporta questo.

**Note tecniche:** due nuove tabelle, `friend_requests` e `guardian_role_requests` (migrazioni `20260918000000`/`20260918010000`), entrambe con RLS a due identità (mittente/destinatario, come già `guardian_verification_requests`) e transizioni di stato SOLO tramite funzioni `security definer` (mai una policy di update per gli utenti: altrimenti il mittente potrebbe scriversi da solo "accepted", falsificando il consenso dell'altro) --- `accept_friend_request`/`reject_friend_request`, `accept_guardian_role_request`/`reject_guardian_role_request`/`revoke_guardian_role`/`resign_as_guardian`. Nuova colonna `friends.is_friend`, mai impostabile direttamente: diventa true solo dentro `accept_friend_request`, che aggiorna in un colpo sia la riga del mittente sia quella del destinatario (se esiste già). Se il destinatario non aveva ancora una propria riga cifrata per il mittente, il client (`acceptFriendRequest`, domain/friends/friend-requests.ts) la crea al volo usando il nome/email presi dalla richiesta (l'email è uno snapshot in chiaro preso dalla sessione del mittente al momento dell'invio --- l'unico modo per il destinatario di conoscerla, non essendo mai stata cifrata per lui). La policy di insert su `guardian_role_requests` verifica `is_friend = true` direttamente nel database (non solo lato client): una PERSONA non può diventare candidata guardiano per costruzione. Riuso del pattern già rodato per la verifica di "Eredità digitale": una policy `profiles_select_by_*` per far leggere al destinatario il nome in chiaro di chi gli scrive, un indice unico parziale (`where status = 'pending'`) per evitare richieste duplicate senza bloccare un nuovo tentativo dopo un rifiuto. Popup in Dashboard (`FriendRequestNotificationPopup`) sullo stesso modello di `SharedCapsuleNotificationPopup`, ma con Accetta/Rifiuta diretti invece di un semplice "ho capito" --- qui serve una vera decisione. Verificato con un nuovo test di integrazione end-to-end contro il database reale (`friend-and-guardian-requests.integration.test.ts`, 11 casi: richiesta/accetta/rifiuta per entrambi i flussi, il blocco lato database di un candidato guardiano non-amico, la simmetria delle righe create) più un giro reale con due sessioni browser separate. Aggiornati/riscritti i test che si appoggiavano al vecchio flusso a interruttore singolo (`friends.spec.ts`, `dashboard-layout.spec.ts`, `onboarding-checklist.spec.ts`, `onboarding-status.spec.ts` --- questi tre forzano guardiano/amicizia via il client admin come dato di partenza, non essendo quello il flusso sotto test).
